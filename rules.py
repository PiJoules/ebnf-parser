...
def main():
    test = "A"
    assert str(Letter.from_str(test)) == test

    test = "9"
    assert str(Digit.from_str(test)) == test

    test = "literal"
    assert str(match_string(test).from_str(test)) == test

    test = "AAA_9__"
    assert str(Identifier.from_str(test)) == test
